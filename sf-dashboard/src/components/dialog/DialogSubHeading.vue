...
const headingValue = computed({
  get: () => dialog.value?.subheading || '',
  set: (value: string) => {
    if (props.campaign?.ad_formats?.unlock_content?.dialog) {
      props.campaign.ad_formats.unlock_content.dialog.subheading = value
    }
    if (props.landing?.unlock_content?.dialog) {
      props.landing.unlock_content.dialog.subheading = value
    }
  },
})

const name = 'Sub Heading'

...
const headingValue = computed({
  get: () => dialog.value?.label_suggested || '',
  set: (value: string) => {
    if (props.campaign?.ad_formats?.unlock_content?.dialog) {
      props.campaign.ad_formats.unlock_content.dialog.label_suggested = value
    }
    if (props.landing?.unlock_content?.dialog) {
      props.landing.unlock_content.dialog.label_suggested = value
    }
  },
})

const name = 'Benefit 2'
const MAX_LENGTH = 80
</script>

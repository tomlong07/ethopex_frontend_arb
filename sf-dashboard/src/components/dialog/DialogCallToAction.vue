...
const headingValue = computed({
  get: () => dialog.value?.call_to_action || '',
  set: (value: string) => {
    if (props.campaign?.ad_formats?.unlock_content?.dialog) {
      props.campaign.ad_formats.unlock_content.dialog.call_to_action = value
    }
    if (props.landing?.unlock_content?.dialog) {
      props.landing.unlock_content.dialog.call_to_action = value
    }
  },
})

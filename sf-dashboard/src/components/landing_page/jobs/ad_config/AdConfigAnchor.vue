...
const status = computed({
  get: () =>
    props.prelander_configs?.anchor ?? {
      status: 'off',
      floor_price: 0,
    },
  set: (value) => {
    if (props.prelander_configs) {
      props.prelander_configs.anchor = value
    }
  },
})
...
<template>
  <div class="flex flex-col gap-4" v-if="props.prelander_configs.anchor">
    <div class="flex items-center">
      <div class="w-40 flex-shrink-0">Anchor</div>
      <div class="flex flex-row place-items-center gap-4 mb-2 flex-1 min-w-0">
        ...
        <div class="flex-1 min-w-0">
          <FloatingWrapper :name="name" medium rounded>
            <n-input-number
              v-model:value="props.prelander_configs.anchor.floor_price"
              :min="0"
              class="flex-1"
            />
          </FloatingWrapper>

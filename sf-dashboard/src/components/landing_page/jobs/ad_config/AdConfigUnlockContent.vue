...
const status = computed({
  get: () =>
    props.prelander_configs?.unlock_content ?? {
      status: 'off',
      floor_price: 0,
    },
  set: (value) => {
    if (props.prelander_configs) {
      props.prelander_configs.unlock_content = value
    }
  },
})
...
watch(
  currentStatus,
  (newValue) => {
    if (!newValue && props.prelander_configs.unlock_content?.dialog) {
      props.prelander_configs.unlock_content.dialog = {}
    }
  },
  { immediate: true }
)
...
<template>
  <div
    ...
    v-if="props.prelander_configs.unlock_content"
  >
    <div class="flex items-center">
      <div class="w-40 flex-shrink-0">Unlock Content</div>
      <div class="flex flex-row place-items-center gap-4 mb-2 flex-1 min-w-0">
        ...
        <div class="flex-1 min-w-0">
          <FloatingWrapper :name="name" medium rounded>
            <n-input-number
              v-model:value="props.prelander_configs.unlock_content.floor_price"
              :min="0"

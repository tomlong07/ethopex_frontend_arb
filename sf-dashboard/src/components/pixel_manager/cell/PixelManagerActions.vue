...
const onDelete = async (link: string) => {
  if (!link) return
  const message = `Are you sure you want to delete pixel ${props.params?.data?.name}?`
  ...
  const result = await ctr_pixel.Remove(link, payload)
  if (result && result.status) {
    ...
    window.message.success('Delete pixel successfully')
  }
}

...
const isShow = computed(() => {
  const isDemand =
    props.campaign.IsDemandAdsense() || props.campaign.IsDemandBing1()

  const isPrelandingOn = props.campaign.prelanding === ONOFF.ON
...
const fetchLandingPageByDemand = async (q = '') => {
  ...
  const res = await ctr_filter_v2.FilterLandingPage(
    {
      q: q,
      f: props.campaign.prelander_landing
        ? String(props.campaign.prelander_landing)
        : undefined,
    },
...
const isDisabled = computed(() => {
  ...
  if (props.campaign.IsCloneCampaign()) {
    return true
  }
...
watch(
  ...
  (visible) => {
    if (visible) {
      ...
      props.campaign.prelander_landing = null
    }
  }
)
...
<template>
  <FloatingWrapper :name="name" rounded v-if="isShow">
    <n-select
      v-model:value="props.campaign.prelander_landing"
      filterable
      remote
      clearable
      :placeholder="name"

...
const submitForm = async (status: AI_STATUS, btn: ButtonLoadingKeys) => {
  ...
  try {
    ...
    if (result?.status) {
      ...
      props.data.campaign.ai_status = status
    }
  } finally {
...
const currentStatusParts = computed(() => {
  const creatives =
    props.data.campaign?.ad_groups?.flatMap(
  ...
  const status = (
    hasManual ? AI_STATUS.MANUAL_REVIEW : props.data.campaign?.ai_status || ''
  ) as AI_STATUS
...
<template>
  <n-card
    ...
    v-if="props.data.FreezeData.isEditPage() && currentStatusParts.type"
  >
    ...
    <div
      ...
      v-if="data.campaign.IsAIStatusRejected()"
    >

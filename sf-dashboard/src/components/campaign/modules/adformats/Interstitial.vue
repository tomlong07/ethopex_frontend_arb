...
const interstitial = computed({
  get: () =>
    props.campaign.ad_formats?.interstitial ?? {
      status: 'off',
      floor_price: 0,
    },
  set: (value) => {
    if (props.campaign.ad_formats) {
      props.campaign.ad_formats.interstitial = value
    }
  },
})

const currentStatus = computed({
  get: () => interstitial.value.status === 'on',
  set: (value: boolean) => {
    interstitial.value = {
      ...interstitial.value,
      status: value ? 'on' : 'off',
  ...
  <div class="flex flex-row place-items-center gap-4">
    ...
    <div class="flex-1 min-w-0">
      <FloatingWrapper :name="name" rounded>
        <n-input-number
          v-model:value="interstitial.floor_price"

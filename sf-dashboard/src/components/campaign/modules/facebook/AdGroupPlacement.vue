...
const descriptionByTag = (tag: PLACEMENT_FB_TAG) => {
  switch (tag) {
    ...
    case PLACEMENT_FB_TAG.MESSAGES:
      return 'Send offers or updates to people who are already connected to your business'
...
const facebookPlacements = computed<SelectOption[]>(() => {
  return [
    ...
    {
      value: PLACEMENT_MESSENGER_POSITION.SPONSORED_MESSAGES,
      label: 'Messenger sponsored messages',
      tag: PLACEMENT_FB_TAG.MESSAGES,
      platform: PLACEMENT_PLATFORM.MESSENGER,
    },
...
const toggleCheckPlatform = (plat: PLACEMENT_PLATFORM) => {
  ...
  if (isCheckPlat(plat)) {
    for (let index = 0; index < itemsInPlatform.length; index++) {
      const it = itemsInPlatform[index]
      if (isDisabledOption(it)) continue
      toggleCheck(it, false, true)
    }
...
const isDisabledOption = (option: SelectOption) => {
  ...
  if (option.value === PLACEMENT_MESSENGER_POSITION.SPONSORED_MESSAGES) {
    return true
  }

  // Disable Business Explore khi location chỉ chọn VN
  if (
  ...
  // Disable Messenger Inbox khi location chỉ chọn US
  if (
  ...
  // Disable Threads Feed nếu không chọn Instagram Feed
  if (
...
const isShow = computed<boolean>(() => {
  if (props.campaign.IsSalesAdvantage()) {
    return false
  }
...
<template>
  <div v-if="isShow && props.campaign.ad_groups" class="flex flex-col gap-4">
    ...
    <div
      ...
      "
    >
      <div class="w-40 font-bold">Platforms</div>
      <div class="flex-1 min-w-0 flex gap-2">
    ...
    <div
      ...
      "
    >
      <div class="w-40 font-bold">Placements</div>
      <div class="flex-1 min-w-0 flex gap-2">

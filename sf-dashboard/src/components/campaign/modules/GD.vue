...
const changeGDFollowUserFlow = () => {
  if (props.campaign.user_flow === '3click') {
...
<template>
  <FloatingWrapper :name="name" rounded v-if="props.campaign.isShowGD()">
    <n-select
      v-model:value="props.campaign.gd"
      filterable
      :placeholder="name"
      :options="gdOptions"
    />

...
const handleEdit = async () => {
  window.router.push({ path: `/telegram-topic/edit/${props.params.data.id}` })
}

const handleDelete = async () => {
  const result = await ctr_topic_telegram.DeleteTopic(props.params.data.id)
   ...
   if (result?.status) {
    ...
    window.message.success('Delete topic successfully')
  }
}
</script>

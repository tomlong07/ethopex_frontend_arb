...
const handleEdit = async () => {
  window.router.push({ path: `/telegram-bot/edit/${props.params.data.id}` })
}

const handleDelete = async () => {
  const result = await ctr_bot_telegram.DeleteBot(props.params.data.id)
   ...
   if (result?.status) {
    ...
    window.message.success('Delete Bot successfully')
  }
}
</script>

...
const dateValue = ref<string[]>([
  reportErrorStore.reportParams.startDate,
  reportErrorStore.reportParams.endDate,
])
const datepicker = ref(null)
const timeZoneOptions = ref<SelectOption[]>([])

const startDateComputed = computed<string>(
  () => reportErrorStore.reportParams.startDate
)
const endDateComputed = computed<string>(
  () => reportErrorStore.reportParams.endDate
)
...
const presetDate = computed<PresetDate[]>(() => {
  const timezone = reportErrorStore.reportParams.timeZone || 'UTC'
  return [
    {
      label: 'Today',
...
const presetSelected = computed<number>(() => {
  ...
  if (startDate === '' && endDate === '') return 5 // Last month
...
const onUpdateReport = () => {
  reportErrorStore.updateClicked += 1
}
...
watch(
  dateValue,
  (v) => {
    reportErrorStore.reportParams.startDate = v[0]
    reportErrorStore.reportParams.endDate = v[1]
    reportErrorStore.changeDate(v[0], v[1])
  },
...
const updateInterval = (value: string) => {
  reportErrorStore.changeInterval(value)
}
const updateTimezone = (value: string) => {
  reportErrorStore.changeTimezone(value)
}
</script>

<template>
  <div
    :class="`date-picker-container flex p-2 border-b ${
  ...
  >
    ...
    <div class="flex justify-start flex-row mr-4">
      ...
      <div class="flex items-center w-24 text-xs">
        <n-select
          v-model:value="reportErrorStore.reportParams.interval"
      ...
      <div class="flex items-center w-60">
        <n-select
          v-model:value="reportErrorStore.reportParams.timeZone"
    ...
    >
      ...
      <n-button
        color="#f43f5e"
        :class="[{ 'mr-2': isMobile }]"
        :loading="reportErrorStore.isFetchingTableUpdate"
...
// Local styles - chỉ áp dụng cho component này
.border-unset {
  border: unset;
}
</style>
...
<style lang="scss">
// Global styles - chỉ cho DatePicker với class container specific
.date-picker-container {
  ...
  // CSS cho n-select trong container này
  .n-base-selection {
